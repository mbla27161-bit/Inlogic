(function () {
    ...
    const cio = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        ...
        function tick(now) {
          ...
          e.target.textContent = Math.round(target * eased).toLocaleString('ru-RU'); // thin spaces for thousands
  ...
  function showModal(title, message, isError) {
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = '<div class="modal-box"><div class="modal-icon" aria-hidden="true"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6L9 17l-5-5"/></svg></div><h3 id="formModalTitle"></h3><p id="formModalMsg"></p><button type="button" class="btn btn-primary" id="formModalClose">Закрыть</button></div>';
      document.body.appendChild(overlay);
      overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.classList.remove('open'); });
      document.getElementById('formModalClose').addEventListener('click', () => overlay.classList.remove('open'));
    }
    const icon = overlay.querySelector('.modal-icon');
    if (isError) {
      icon.innerHTML = '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 8v5M12 16h.01"/></svg>';
      icon.style.background = 'rgba(200,80,60,.15)';
      icon.style.color = '#c45c4a';
    } else {
      icon.innerHTML = '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6L9 17l-5-5"/></svg>';
  ...
  document.querySelectorAll('.phone-field input[data-phone-local], .phone-field input[name="phone"]').forEach(input => {
  ...
  async function sendViaEmailJS(payload) {
    ...
    if (window.emailjs && typeof window.emailjs.send === 'function') {
      if (typeof window.emailjs.init === 'function') {
        try { window.emailjs.init({ publicKey: pub }); } catch (_) {}
      }
  ...
  document.querySelectorAll('.quote-form').forEach(form => {
    ...
    form.addEventListener('submit', async function (e) {
      ...
      if (btn) {
        ...
        btn.innerHTML = '<span class="btn-spinner" aria-hidden="true"></span> Отправляем заявку…';
      }

      const fd = new FormData(this);
      const phoneInput = this.querySelector('[name="phone"], [data-phone-local]');
      ...
      try {
        const configured = !!(EMAILJS_CFG.publicKey || EMAILJS_CFG.userId) && EMAILJS_CFG.serviceId && EMAILJS_CFG.templateId;
        if (configured) {
          await sendViaEmailJS(payload);
        } else {
          await new Promise(r => setTimeout(r, 500));
          console.info('[InLogic form] Configure window.INLOGIC_EMAILJS in config.js', payload);
        }
        this.reset();
        // restore phone input empty after reset
        if (phoneInput) phoneInput.value = '';
        lockedUntil = Date.now() + FORM_COOLDOWN_MS;
        showModal('✓ Заявка успешно отправлена', 'Спасибо! Персональный менеджер свяжется с вами в ближайшее время.', false);
      } catch (err) {
        console.error(err);
        showModal('Не удалось отправить', 'Проверьте соединение и попробуйте ещё раз или позвоните нам.', true);
      } finally {
        if (btn) {
          setTimeout(() => {
            ...
            if (btn.disabled) {
              ...
              setTimeout(() => { btn.disabled = false; }, left);
            }
          }, 400);
        }
      }
  ...
  document.querySelectorAll('.career-split, .svc-panel').forEach(panel => {
    const btns = panel.querySelectorAll('.career-split-nav button, .svc-panel-nav button');
  ...
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    document.querySelectorAll('animateMotion').forEach(el => {
      try { el.endElement && el.endElement(); } catch (_) {}
  ...
  (function initIntro() {
    ...
    let seen = false;
    try { seen = sessionStorage.getItem('inlogic-intro') === '1'; } catch (_) {}
    ...
    setTimeout(() => {
      ...
      try { sessionStorage.setItem('inlogic-intro', '1'); } catch (_) {}
    }, fadeAt);
